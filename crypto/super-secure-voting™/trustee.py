...
class Server:
    ...
    def election(self, c):
        ...
        try:
            ...
            votes = [paillier_decrypt_m_from_r(c, r, self.N) for c, r in zip_longest(enc_votes, dec_vote_r)]

            if votes[0] > votes[1]:
                c.sendall(b"Take that, election rigging Stan!\n")
                c.sendall(self.flag_share.hex().encode() + b"\n")
            else:
                c.sendall(b"Looks like we're in for another year of Stan's tyranny.\n")
