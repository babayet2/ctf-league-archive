...
void init_game(struct gameInterface *game){
    ...
    snprintf(game->command, BUFFER, "/usr/games/gnuchess -g -m %s", arguments);
}

int main(){
    printf("WELCOME TO THE NORWEGIAN COLLEGE OF ELITE PWN\n");
    printf("Please enter your custom command line arguments, or just hit enter to begin your training: ");
    struct gameInterface *game = malloc(sizeof(struct gameInterface));
    init_game(game);
    printf("executing command: %s\n", game->command);
    launch_gnuchess(game);
    thread_handler(game);
    printf("\nfinished executing command %s\n", game->command);
    free(game);
    return 0;
}
